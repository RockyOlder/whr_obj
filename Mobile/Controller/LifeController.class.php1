...
class LifeController extends Controller {
    ...
    public function info(){  
        ...
        if (IS_POST) {
            ...
            $sql = "insert into `wrt_admin` set `salt`=$data[salt],`name`='$data[name]',
                ...
                `top_name`='慧享园-生活服务商家管理系统',`statue`=2,`flag`=1";
                // dump($sql);
            $uid=M()->execute($sql);
            // dump($data);
            // $uid = 1;
            if($uid){
                $uid = M('admin')->field('id')->where(array('name'=>$data['name'],'shop_id'=>$bool))->find();
                // dump($uid);
                $bool2 = $uid['id'];
            } 
            $role['group_id'] = 5;
    ...
    public function sendMsg(){
    	$code = cookie('mobile_time')+60;
    	if($code > time())$this->ajaxReturn('验证码已经发送，如果没有收到请检查你的手机号码刷新页面重新获取');
    	$mobile = I('request.mobile');
        //给客户手机发送验证码
        $code = mt_rand(99999,999999);
		cookie($mobile,$code,60);//存入手机验证码
        cookie('mobile',$mobile,60);//存入手机号码
		cookie('mobile_time',time(),60);
