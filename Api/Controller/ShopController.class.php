               ...
               // 获取商店的评论3条
                ...
                if (!empty($comm)) {
                  ...
                  foreach ($comm as $key => $value) {
                    // 查找用户头像
                    $sql = "select face,rank_name from ".C('DB_PREFIX')."user as u join ".C('DB_PREFIX')."user_rank as k on u.user_rank =k.rank_id  where user_id = $value[user_id]";
                    // dump($sql);
                    $data = M()->query($sql);
                    // dump($data);
                    if (!empty($data)) {
                       $user = current($data);
                        $value['face'] =$user['face'];
                        $value['rank_name'] = $user['rank_name'];
                    }
                    $comm[$key] = $value;
                  }
                }
                ...
                // dump($comm);
              // 获取推荐商店
                $sql = "select b.id,name,des,star,type,list_pic,latitude,longitude from ".C('DB_PREFIX')."give_life_shop as g join ".C('DB_PREFIX')."business as b on g.shopid =b.id where deadline > ".time()." order by g.sort desc limit 0,3";
                // dump($sql);
                $give = M()->query($sql);

                foreach ($give as $key => $value) {
                    $sql = "select type_name from ".C('DB_PREFIX')."type where type_id =".$value['type'];
                    // dump($sql);
                    $data = M()->query($sql);
                    if (!empty($data)) {
