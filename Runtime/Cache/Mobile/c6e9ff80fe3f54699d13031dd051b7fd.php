...
<div class="menuset">
 ...
 <div style="text-align:center;font-size:25px ;">慧享园的生活导航合作商家信息登记</div>
<div class="conten">
 
<div class="content">
  <div class="name">
  <span class="tt">店铺分类：</span>
  <span class = 'pro'>
      <select name = 'parent_type' id="type_on" class="dfinputInfo"style=" width:105px;" reg="[^0]" tip="一定要选择哟">
          <option class="pro_category"  value="0">请选择</option>
      ...
      <select name = 'type' style="display:none; width:105px;margin-top:15px;" id ="son_cate" class="form-control" reg="[^0]" tip="一定要选择哟">
  ...
  <input name="name" type="text"  value=""  id="flightno" reg="" tip=" 商家名称 如: 慧锐通" url="<?php echo U('ajax_check_vip','','');?>" style=" padding:5px 50px 5px 5px;">
  ...
  <input name="user_name" type="text"  value="" id="flightno" reg="^\W{2,20}$" tip="店主姓名" style=" padding:5px 50px 5px 5px;">
  <input type="hidden" name ="user_id" value="<?php echo ($_SESSION['admin']['aid']); ?>">
...
<div class="postal" style="both:clear;margin-top:20px;">
  ...
  <div class="postal" style="both:clear;margin-top:20px;">
  <span class="tt">百度经度：</span>
  <input name="longitude" type="text" id="flightno" reg="" tip="经度"  style=" padding:5px 50px 5px 5px;">
  <p>提示：百度地图后面为纬度前面为经度</p>
  </div>
  <div class="postal" style="both:clear;margin-top:20px;">
  <span class="tt">百度纬度：</span>
  <input name="latitude" type="text" id="flightno" reg="" tip="纬度"  style=" padding:5px 50px 5px 5px;">
  <p>提示：百度地图后面为纬度前面为经度</p>
  </div>
<div class="describe">商家描述：<br/><textarea name="des" cols="27" rows="5" style=" margin:20px 0px 0px 80px" id="flightno" reg="\W{3,50}" tip="商家描述不能小于四个字大于50个字"></textarea></div>


<div class="tit" style=" border-bottom:1px dashed #CCC; margin-top:50px;"><h2 style="color:#999;">商家基本资料</h2></div>
<div class="photo">
...
<div class="con-con">
<div class="aaa" style="float:left">店面列表图片：</div>
<div id="list_pic" style="height:100px;">
    

</div>
<input type="file" id = "upload_list" onChange="previewImage(this)">
<input type="hidden" name = 'list_pic' value="">
<input type="hidden" name = 'list_path' value="">
<input type="hidden" name = 'mid_pic' value="">
</div>
<div class="con-con">
<div class="aaa" style="float:left">店铺详细图片（多张）：</div>
<div id="more_pic" style="height:auto;padding-bottom:50px;overflow: hidden;">
    

</div>
<input type="file" id = "upload_photo" onChange="previewImage(this)">
</div>
<div class="con-con">
...
<script type="text/javascript">
    $('#upload_list').uploadify({
        ...
        'onUploadSuccess' : function(file, data, response) { 
            ...
            //alert(obj.min);
            pic = "<img height='100px' src='"+obj.min+"'>";
            // pic +=" <img src='/Mobile/View/Public/images/uploadify-cancel.png' class ='close' onclick = 'javascript:deleteListPic()'> "
            $('#list_pic').html(pic);
            $('input[name=list_pic]').val(obj.min);
            $('input[name=list_path]').val(obj.path);
            $('input[name=mid_pic]').val(obj.mid);
        ...
        'onUploadSuccess' : function(file, v, response) {
            ...
            img += "<img height='60px' src='"+obj.path+"' name ='path'>";
            ...
            img += "<img src='/Mobile/View/Public/images/uploadify-cancel.png' class ='close' onclick = 'javascript:deletePic("+num+")'> ";
            img += "</div>"
            $('#more_pic').append(img);
            img = '';
            num++;
